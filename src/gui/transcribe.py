...
from ..forms.transcribe import Ui_Dialog
...
class TranscribeDialog(Dialog):
    ...
    def setup_ui(self) -> None:
        ...
        self.provider: Provider = init_provider(self.provider_class)
        self.provider_widget: ProviderWidget | None = None
        self.lang = config["lang_field"]
        ...
        try:
            idx = provider_names.index(config["provider_field"])
    ...
    def on_provider_changed(self, index: int) -> None:
        ...
        self.provider = init_provider(self.provider_class)
    ...
    def _fill_fields(self) -> bool:
        ...
        try:
            idx = field_names.index(config["audio_field"])
            self.form.audioField.setCurrentIndex(idx)
        except ValueError:
            pass
        try:
            idx = field_names.index(config["text_field"])
    ...
    def on_finished(self) -> None:
        ...
        config["provider_field"] = provider_field
        config["lang_field"] = lang_field
        config["provider_options"][self.provider_class.name] = dataclasses.asdict(
